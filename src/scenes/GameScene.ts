...
type Btn = Phaser.GameObjects.Sprite & {
  customStatus?: Stats
}
...
export class GameScene extends Phaser.Scene {
  ...
  stats!: Stats
  uiBlocked = false
  selectedItem: Btn | null = null
  healthText!: Phaser.GameObjects.Text
  funText!: Phaser.GameObjects.Text
  timedEventStats!: Phaser.Time.TimerEvent
  decayRates!: Stats
  ...
  rotatePet(this: Btn, pointer: Phaser.Input.Pointer) {
    const scene = this.scene as GameScene
    if (scene.uiBlocked) return
    scene.uiReady()
  ...
  pickItem(this: Phaser.GameObjects.Sprite, pointer: Phaser.Input.Pointer) {
    const scene = this.scene as GameScene
    if (scene.uiBlocked) return
    scene.uiReady()
  ...
  updateStats(statDiff: Stats) {
    ...
    for (const stat of Object.keys(statDiff) as (keyof typeof statDiff)[]) {
